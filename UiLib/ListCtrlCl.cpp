...
void CListCtrlCl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	for ( int nCol=0; GetColumn(nCol, &lvc); nCol++)
	{
		...
		{
            COLORREF bkclr = m_bFocus?  RGB(134,211,255)  : RGB(200,245,255);
			pDC->FillSolidRect(&rcTemp, bkclr);//GetSysColor(COLOR_HIGHLIGHT)) ;
			pDC->SetTextColor(GetSysColor(COLOR_HIGHLIGHTTEXT)) ;
            TRACE("test %X, %d\n",lpDrawItemStruct->itemState,m_bFocus );
		}
		else
		{
            TRACE("test %X, %d\n",lpDrawItemStruct->itemState,m_bFocus );
        ...
        rcItem.left += 4;
		DrawText(lpDrawItemStruct->hDC, lpBuffer, (int)_tcslen(lpBuffer), &rcItem, uFormat) ;
...
void CListCtrlCl::OnKillFocus(CWnd* pNewWnd)
{
    m_bFocus = 0;
...
void CListCtrlCl::OnSetFocus(CWnd* pOldWnd)
{
    m_bFocus = 1;
