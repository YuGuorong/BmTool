...
void CHeaderCtrlCl::OnPaint()
{
	...
	CBitmap btmp;
	btmp.LoadBitmap(IDB_BTMP_BK_HDR);
	CDC mdc;
	mdc.CreateCompatibleDC(&dc);
	mdc.SelectObject(&btmp);

	//dc.StretchBlt(0,0,clientRect.Width(),clientRect.Height(),&mdc,0,0,33,30, SRCCOPY);
    dc.StretchBlt(0,0,clientRect.Width(),clientRect.Height(),&mdc,0,0,33,clientRect.Height(), SRCCOPY);
	int nItem; 
	...
	{ 
		...
		CBrush  brush(RGB(124,240,240));
	//	dc.FrameRect(&tRect,&brush); //填充背景 
		...
		dc.SetTextColor(RGB(0,0,0));//m_color);
