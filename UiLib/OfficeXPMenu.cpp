...
void COfficeXPMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
        int len = sizeof(wchar_t)*((int)wcslen(szWideString)+1);
...
BOOL COfficeXPMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		nLen =(int)wcslen((wchar_t *)pTp)+1;
		szCaption=new wchar_t[nLen];
		wcscpy_s(szCaption,nLen,(wchar_t *)pTp);
...
int COfficeXPMenu::RemoveMenu(char* pText, BC_Seperator sPos)
{
	USES_CONVERSION;
	return RemoveMenu(A2W(pText), sPos);
}

int COfficeXPMenu::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
